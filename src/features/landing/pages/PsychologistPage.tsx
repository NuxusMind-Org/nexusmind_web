...
export const PsychologistPage = ({ psychologistId }: PsychologistPageProps) => {
  const psych = psychologists.find(p => p.id === psychologistId) || psychologists[0];

  return (
    <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-[#774885] via-[#2b4c5e] to-[#213542] overflow-y-auto pt-32 flex flex-col items-center z-40">
      
      {/* Main Content Container */}
      <div className="w-full max-w-[1200px] px-6 sm:px-12 md:px-20 flex flex-col gap-8 flex-1 mb-20">
        
        <h1 className="text-[32px] sm:text-[42px] md:text-[56px] font-serif text-white mb-2 leading-tight text-center lg:text-left mt-4 sm:mt-0">
          Psixoloq haqqında
        </h1>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 flex flex-col gap-6">
            ...
            {/* Profile Card */}
            <div className="bg-white/10 backdrop-blur-md rounded-[24px] p-6 sm:p-8 border border-white/10 shadow-xl flex flex-col sm:flex-row gap-6 sm:gap-8 relative items-center sm:items-start text-center sm:text-left">
              <div className="relative shrink-0">
                ...
                <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-[#00f2ff] text-[#111] px-3 py-1 rounded-full text-[12px] font-bold flex items-center gap-1 shadow-md">
              ...
              <div className="flex flex-col flex-1 w-full">
                <div className="flex flex-col sm:flex-row justify-between items-center sm:items-start mb-2 gap-3 sm:gap-0">
                  <div>
                    <h2 className="text-[24px] sm:text-[32px] font-bold text-white leading-tight">{psych.name}</h2>
                    <p className="text-white/70 text-[14px]">{psych.experience} • {psych.title}</p>
                  </div>
                  <div className="text-[#00f2ff] font-bold text-[24px] sm:text-[28px] text-center sm:text-right">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 justify-center sm:justify-start mt-auto">
                  {psych.languages.map(lang => (
                    <span key={lang} className="px-3 sm:px-4 py-1.5 rounded-full border border-white/20 bg-white/5 text-white/80 text-[12px] sm:text-[13px]">{lang}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              {/* Education */}
              <div className="flex-1 bg-white/10 backdrop-blur-md rounded-[24px] p-6 sm:p-8 border border-white/10 shadow-xl">
                <h3 className="text-white text-[18px] font-medium flex items-center gap-2 mb-6">
                ...
                <div className="flex flex-col gap-5">
                  {psych.education.map((edu, i) => (
                    <div key={i} className="border-l-2 border-white/20 pl-4">
                      <h4 className="text-white font-medium mb-1">{edu.uni}</h4>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-[24px] p-6 sm:p-8 border border-white/10 shadow-xl">
              <h3 className="text-white text-[18px] font-medium mb-6">İxtisaslaşdığı sahələr</h3>
              <div className="flex flex-wrap gap-3">
                {psych.tags.map(tag => (
                  <span key={tag} className="px-4 py-2 rounded-xl bg-white/10 border border-white/10 text-white/90 text-[14px]">{tag}</span>
                ))}
              </div>
            </div>
          ...
          <div className="w-full lg:w-[350px] xl:w-[400px] flex flex-col gap-6">
            ...
            {/* Booking Box */}
            <div className="bg-[#2D3E50]/60 backdrop-blur-xl rounded-[24px] p-6 sm:p-8 border border-white/10 shadow-xl flex flex-col">
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-[#00f2ff]/20 text-[#00f2ff] text-[10px] font-bold px-2 py-1 rounded-bl-lg tracking-wider">TEZLİKLƏ</div>
              ...
              <button className="w-full py-4 bg-[#c084fc] hover:bg-[#a855f7] text-[#1e1b4b] font-bold text-[16px] rounded-xl transition-colors shadow-lg">
                Seans Təyin Et
              </button>
            ...
            <div className="w-full h-[180px] rounded-[24px] overflow-hidden relative group cursor-pointer shadow-xl border border-white/10">
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-[#eeb3b3]/30 backdrop-blur-xl border border-white/20 rounded-[16px] p-4">
                <h4 className="text-[14px] font-bold text-[#111] mb-1">VR KONSULTASİYA</h4>
                <p className="text-[9px] text-[#222] font-semibold line-clamp-2">Burada evdən çölə çıxmadan istədiyin konfort zonanı seçə və orada zaman keçirərək sakitləşə bilərsən.</p>
              </div>
            </div>
